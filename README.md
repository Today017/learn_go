...
1. `main.go` の中で、ハンドラ層の関数とパスの関連付けを行う
2. ハンドラ層から、サービス層の関数を呼び出す
3. 各サービス関数の中で都度 `sql.DB` 型（Go公式で非推奨）を生成し、データベース関連の処理を行う


after

サービス構造体 `MyAppService` とコントローラ構造体 `MyAppController` を導入

この変化により、サービス層と `sql.DB` 型との関係について、
サービス関数が呼ばれる→`sql.DB` が作られるという順であり、サービス層の処理がデータベースに依存していたが、

サービス構造体の導入により、「`sql.DB` を作る→作った `sql.DB` を基にしたサービス構造体を作る」という流れ人った。

サービス構造体で作る `sql.DB` を引数を使って外から渡す形にしたことで、サービス構造体の役割が「引数で渡された `sql.DB` 方を使ってやりたい処理をする」という実装になり、
サービス層内部からのデータベースへの依存をなくすことができる。

このように、ある処理の内部で使うコンポーネントを外から渡してやるような実装パターンのことを依存性の注入という。
...
`MyAppController` のメンバ変数として `MyAppService` 構造体を要求しているが、この部分を interface を用いて抽象化する。
Goでは「インターフェースのポインタを取る」事ができないので注意。

抽象化する以前は `MyAppController` を作るためには `MyAppService` 構造体が必要だったが、このように「ある特定の型」に依存している状態は良くない。
例）PostgresSQL を使ったデータベース処理を実装したサービス型 `AnotherService` 型を新たに実装して差し替えたくなったときなど

### 6-5

- 6-4で行ったインターフェースによる抽象化を更に細分化して抽象度を上げた。
- それにともなって、controllerを `Article` 担当と `Comment` 担当に分離した。
- ルーター作成部分の引数を `sql.DB` に変更し、中で `MyAppService` や `Controller` を作成するようにした（ルーター層の役割を増やした）
    - ルーターより役割が増えたので `api` に変更
- mainの中では重要なコードだけが記述されるようになった
    - 使用するデータベースの指定（ポート番号、アドレス、エンジン）
    - APIサーバーの起動
...
- エラーチェーン
    - そのエラーの原因となったエラーを入れるための入れ子構造

### 7-2

- `errors.Is` 関数
    - `==` だと、エラーがラップされている場合にうまくいかない

### 7-3
- `errors.New` 関数
    - エラー情報を乗せた文字列のみのシンプルなエラーを作ることができる

### 7-4

- `errors.As` 関数
    - error の型変換用の関数
    - エラーチェーンに含まれるエラーを探索して第二引数の型に合うものを持ってきてくれる

### まとめ

- `Error() string` メソッドを持つ構造体は error インターフェースを満たす
- エラー構造体のフィールドにそのエラーの原因のエラーを含まていくことでエラーチェーンを作れる

## 8
### 8-2
- interfaceによる抽象化を利用して簡単にMock構造体を作れる